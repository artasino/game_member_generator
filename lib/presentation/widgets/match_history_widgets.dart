...
class _MatchSettingsDialogState extends State<MatchSettingsDialog> {
  ...
  int? get _excludingSessionIndex =>
      widget.isRecalc ? widget.currentSession?.index : null;

  List<MatchType> get _selectedTypes =>
      isAutoRecommendEnabled ? _buildAutoTypes() : types;

  void _clearSubmitMessage() {
    submitValidationMessage = null;
  }

  RequirementResult _checkRequirements({
    required List<MatchType> targetTypes,
    required bool performAlgorithmCheck,
  }) {
    return widget.notifier.checkRequirements(
      targetTypes,
      performAlgorithmCheck: performAlgorithmCheck,
      excludingSessionIndex: _excludingSessionIndex,
    );
  }
  ...
  void _handleStart() {
    ...
    final finalCheck = _checkRequirements(
      targetTypes: _selectedTypes,
      performAlgorithmCheck: true,
    );
  ...
  Widget build(BuildContext context) {
    ...
    final precheck = _checkRequirements(
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      children: MatchType.values.map((type) {
                        ...
                        final canAddType = _checkRequirements(
                          targetTypes: tentativeTypes,
                          performAlgorithmCheck: false,
                        ).canGenerate;
                        return ActionChip(
                          label: Text(type.displayName,
