...
class _MatchHistoryScreenState extends State<MatchHistoryScreen> {
        ...
        return Scaffold(
          appBar: AppBar(
            ...
            title: isEmpty
                ...
                : (!isSwapping
                    ? _buildNormalHeaderContent(
                        theme, session!, sessions.length)
            ...
            children: [
              if (!isEmpty) ...[
                FloatingActionButton.small(
                  ...
                  onPressed: widget.notifier.isGenerating
                      ? null
                      : () => _showSettingsAndGenerate(context,
                          isRecalculate: true),
                  tooltip: '再生成',
              ...
              FloatingActionButton.extended(
                ...
                onPressed: widget.notifier.isGenerating
                    ? null
                    : () =>
                        _showSettingsAndGenerate(context, isRecalculate: false),
  ...
  Widget _buildGamesArea(
      Session session, PlayerStatsPool pool, double scale, double screenWidth) {
    final int gameCount = session.games.length;
    ...
    return Wrap(
      ...
      children: session.games.asMap().entries.map((entry) {
        return SizedBox(
          width: cardWidth,
          child: _buildGameCard(context, entry.key, entry.value,
              Theme.of(context), pool, scale, session),
        );
      }).toList(),
    );
  }
  ...
  Widget _buildNormalHeaderContent(
      ThemeData theme, Session session, int total) {
  ...
  Widget _buildGameCard(BuildContext context, int index, Game game,
      ThemeData theme, PlayerStatsPool pool, double scale, Session session) {
    ...
    return Card(
      ...
      child: Column(
        children: [
          ...
          Padding(
            padding: EdgeInsets.fromLTRB(
                10 * scale, 12 * scale, 10 * scale, 10 * scale),
            child: Row(
              children: [
                Expanded(
                    child: _buildTeamColumn(
                        session, game.teamA, pairCountA, scale)),
                Padding(
                ...
                Expanded(
                    child: _buildTeamColumn(
                        session, game.teamB, pairCountB, scale)),
              ],
            ),
          ),
        ],
  ...
  Widget _buildTeamColumn(
      Session session, Team team, int pairCount, double scale) {
    return Column(
      children: [
        _PlayerTag(
          player: team.player1,
  ...
  Widget _buildRestingContainer(
      Session session, ThemeData theme, double scale, double maxWidth) {
        ...
        children: [
          ...
          Row(
            ...
            children: [
              if (males.isNotEmpty)
                Expanded(
                  child: _RestingSubSection(
                      label: '男性',
                      players: males,
                      color: Colors.blue,
                      onPlayerTap: (p) => _handlePlayerTap(session, p),
                      onPlayerLongPress: (p) => _handlePlayerLongPress(p),
  ...
  Future<void> _handlePlayerTap(
      Session currentSession, Player clickedPlayer) async {
  ...
  Future<void> _swapPlayers(Session session, Player p1, Player p2) async {
    ...
    await widget.notifier.updateSession(
  ...
  void _showSettingsAndGenerate(BuildContext context,
      {required bool isRecalculate}) {
    widget.notifier.getCurrentSettings().then((currentSettings) {
      if (!mounted) return;
      final sessions = widget.notifier.sessions;
      final currentSession =
          (isRecalculate && _currentIndex != null && sessions.isNotEmpty)
              ? sessions[_currentIndex!]
              : null;
      final initialMatchTypes =
          currentSession?.games.map((g) => g.type).toList();
                      ...
                      if (selectedTypes.isNotEmpty) ...[
                        Wrap(
                            ...
                            children: selectedTypes
                                ...
                                .map((entry) => Chip(
                                    label: Text(
                                        _matchTypeName(entry.value)
                                            .replaceAll('ダブルス', 'W'),
                                        style: const TextStyle(fontSize: 12)),
                                    onDeleted: () => setState(() =>
                                        selectedTypes.removeAt(entry.key)),
                      ...
                      onPressed: (!res.canGenerate || selectedTypes.isEmpty)
                          ...
                          : () async {
                              ...
                              if (isRecalculate && currentSession != null) {
                                _recalculateSession(currentSession.index,
...
class _PairInfoLabel extends StatelessWidget {
  ...
  const _PairInfoLabel(
      {required this.count, required this.team, required this.scale});
    ...
    return Container(
      ...
      child: Text('ペア $count回目',
          style: TextStyle(
              ...
              fontWeight: isMultiple ? FontWeight.w900 : FontWeight.bold)),
    );
  }
}
    ...
    return GestureDetector(
      ...
      child: Container(
          ...
          decoration: BoxDecoration(
              ...
              border: Border.all(
                  color:
                      isSelected ? Colors.orange : color.withValues(alpha: 0.5),
